...
const manualTestPlanPreview = require('../services/manualTestPlanPreviewService');
...
// best/worst narrow-state strategies from the Learning Engine. Never trades.
router.get('/supervisor/narrow-state', (req, res) => {
  try {
    const minScore = Number.isFinite(Number(req.query.minScore)) ? Number(req.query.minScore) : 60;

    let stockResults = [];
    let cryptoResults = [];
    try { stockResults = getLatestResults() || []; } catch (_) { stockResults = []; }
    try { cryptoResults = getCryptoResults() || []; } catch (_) { cryptoResults = []; }
    const allResults = [...stockResults, ...cryptoResults];

    const overview = narrowStateEngine.summarizeNarrowState(allResults, { minScore });

    // Narrow-state strategy catalog (family === 'narrow_state')
    let catalog = [];
    try { catalog = (daytradingStrategyCatalog.getCatalog().strategies || []); } catch (_) { catalog = []; }
    const narrowStrategies = catalog.filter((s) => s.family === 'narrow_state');

    // Performance Learning summary is the source of truth for narrow performance.
    let performanceLearning = null;
    let bestStrategy = null;
    let worstStrategy = null;
    let bestScoreBand = null;
    let strongestConfirmation = null;
    let recommendedNextTest = null;
    let dataConfidence = 'none';
    let status = 'needs_more_data';
    let message = 'Systemet har ännu för lite Narrow State-data för säker slutsats.';
    let latestLessons = [];
    try {
      const hours = Number.isFinite(Number(req.query.hours)) ? Number(req.query.hours) : 168;
      const summary = daytradingLearning.buildLearningSummary({ hours });
      if (Array.isArray(summary.lessons)) latestLessons = summary.lessons.slice(0, 5);
      else if (Array.isArray(summary.notes)) latestLessons = summary.notes.slice(0, 5);
    } catch (_) { /* learning summary optional */ }
    ...
    if (performanceLearning) {
      ...
      bestScoreBand = performanceLearning.bestScoreBand;
      strongestConfirmation = performanceLearning.strongestConfirmation;
      recommendedNextTest = performanceLearning.recommendedNextTest;
      dataConfidence = performanceLearning.dataConfidence || 'none';
      status = performanceLearning.status || 'needs_more_data';
      message = performanceLearning.message || 'Systemet har ännu för lite Narrow State-data för säker slutsats.';
    }
    ...
    res.json({
      ...
      narrowState: {
        ...
        strongCompressionCount: overview.strongCompressionCount,
        topSymbols: overview.topSymbols,
        strongestCompression: overview.strongestCompression,
        breakoutWatch: overview.breakoutWatch,
        fakeoutRisk: overview.fakeoutRisk,
        meanReversion: overview.meanReversion,
        strategies: narrowStrategies.map((s) => ({
          id: s.id, name: s.name, family: s.family, version: s.version,
          status: s.status, enabled: s.enabled, paper_only: s.paper_only,
          live_enabled: s.live_enabled, learning_tags: s.learning_tags,
        })),
        bestStrategy,
        worstStrategy,
        bestScoreBand,
...
router.get('/strategies/test-queue/:id/preview', (req, res) => {
  try {
    const result = manualTestPlanPreview.defaultManualTestPlanPreviewService.getTestPlanPreview(req.params.id);
    const status = result.ok ? 200 : (result.error === 'queue_item_not_found' ? 404 : 400);
    res.status(status).json(result);
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message, ...manualTestPlanPreview.SAFETY });
  }
});
...
// ── IB Paper — Futures Contract Discovery (read-only, Phase 3.3) ─────────────
// Returns the merged futures live view: static Fas 1 spec + (gated) Fas 3.1
// front-month resolver + (gated) Fas 3.2 market-data snapshot.
// Both data sources are GATED OFF by default (IB_FUTURES_CONTRACT_DETAILS_ENABLED
// / IB_FUTURES_MARKETDATA_ENABLED). With both off NO IB contact happens and the
// payload degrades to the exact Phase-1 static behaviour. Read-only: front-month
// and price stay unverified when gated, every contract is isTradablePreview=false,
// there is NO order path here, and submit stays hard-gated by
// IB_PAPER_SUBMIT_ROUTES_ENABLED.
router.get('/interactive-brokers/futures/contracts', async (req, res) => {
  try {
    res.json(await interactiveBrokersFuturesLiveViewService.buildFuturesLiveView());
  } catch (err) {
    // Fail safe: never surface a broken futures view — fall back to the pure
    // Phase-1 static spec so the read-only endpoint stays available.
    try {
