...
const DEFAULT_SYMBOLS = Object.freeze({
  ...
  sp500: ['SPY', 'AAPL', 'MSFT'],
  // Futures saknades i den här tabellen, och det var inte harmlöst: de enda
  // strategier systemet faktiskt kan EXEKVERA är futures-strategier, och de
  // föll därför på `unknown` och fick QQQ/SPY. Ett jobb för
  // native_futures_vwap_failed_breakout_short_v1 schemalades mot aktier och
  // kunde per konstruktion aldrig säga något om strategin.
  futures: ['MES', 'MNQ'],
  unknown: ['QQQ', 'SPY'],
});

// Rötter och etiketter som gör en strategi till en futures-strategi.
const FUTURES_TAGS = Object.freeze(new Set(['futures', 'mnq', 'mes', 'nq', 'es', 'globex', 'cme']));
...
// ── Vilken timeframe jobbet KOMMER att köras i ───────────────────────────────
//
// En enda definition, läst av både schemaläggaren (som bygger jobbet) och AI
// Factory-orchestratorn (som bygger optimerarens experimentkontext). Fanns den
// på två ställen skulle de förr eller senare svara olika på samma fråga — och
// då hade optimeraren bokfört ett experiment i en annan timeframe än den replay
// faktiskt körde, vilket är exakt det fel AI Memory v2 finns för att fånga.
//
// Returnerar null när INGENTING anger en timeframe. Motorns förval (2m) läggs
// medvetet inte på här: ett förval som slår till vid exekvering är inte kunskap
// som finns vid förslagstillfället, och att påstå det hade varit att hävda
// något ingen observerat. Den som inte vet får en v1-identitet, vilket är rätt.
const DEFAULT_JOB_TIMEFRAME = '2m';

function resolveJobTimeframe({ gap = null, strategy = null, job = null, config = null } = {}) {
  const candidate = job?.execution_model?.timeframe
    || job?.market_dna?.timeframe
    || config?.timeframe
    || gap?.timeframe
    || strategy?.timeframe
    || null;
...
function inferMarketGroup(strategy = {}, score = {}, gap = {}) {
  ...
  // Futures prövas före krypto och index: en strategi taggad både "futures"
  // och "index" är en indexTERMIN, inte en indexaktie, och ska köras på CME.
  if (tags.some((tag) => FUTURES_TAGS.has(tag))) return 'futures';
...
function buildJob({ strategy, score, history, gap, coverageRows, now, requestedBy, periodDays }) {
  ...
  const marketDna = {
    ...
    timeframe: resolveJobTimeframe({ gap, strategy }) || DEFAULT_JOB_TIMEFRAME,
    dna_tags: [
      ...safeArray(strategy.market_regime_tags),
      ...safeArray(strategy.tags),
      ...safeArray(gap?.tags),
  ...
  return {
    ...
    // Genomet kunskapsluckan bär med sig, om cykeln just muterat fram ett.
    // Kön gör det till en del av jobbets identitet och körningen garanterar att
    // just det genomet prövas — se normalizeGenome i replayQueueService.
    genome: genomeFrom(gap),
  };
}

/**
 * Genomet ur kunskapsluckan.
 *
 * Fabrikscykeln skapar ett genom i CREATE_DNA_GENERATION och schemalägger sedan
 * en replay. Utan den här kopplingen var de två stegen obesläktade: jobbet
 * beskrev en strategi, inte det genom som just skapats för att prövas.
 */
function genomeFrom(gap) {
  const raw = gap?.genome || gap?.Genome;
  if (!raw) return null;
  const dnaHash = safeString(raw.dnaHash || raw.dna_hash);
  if (!dnaHash) return null;
  return {
    dna_hash: dnaHash,
    strategy_id: safeString(raw.strategyId || raw.strategy_id) || null,
    root_strategy_id: safeString(raw.rootStrategyId || raw.root_strategy_id) || null,
    parent_dna_hash: safeString(raw.parentDnaHash || raw.parent_dna_hash) || null,
...
function config() {
  return {
    // PÅ som standard sedan fabrikscykeln är komplett. Flaggan stängde
    // tidigare av en schemaläggare vars jobb ingen körde — att ha den på hade
    // bara fyllt en kö. Nu driver den hela kedjan, och kedjan är paper_only
    // hela vägen: ingen broker, inga order, replay i barnprocess, enbart
    // append-only-skrivningar. ENABLE_REPLAY_SCHEDULER=false stänger av den.
    enabled: bool(process.env.ENABLE_REPLAY_SCHEDULER, true),
...
function createReplaySchedulerService(options = {}) {
  ...
  // ── Två register, en schemaläggningspopulation ─────────────────────────────
  //
  // Strategy Brain väljer ur BIBLIOTEKET, som är nycklat på både katalogens och
  // native-registrets id:n. Schemaläggaren slog bara upp katalogen, hittade
  // ingen rad för ett native-id, och byggde då en tom stubbrad utan marknad —
  // vilket är exakt hur en futures-strategi hamnade på QQQ/SPY.
  //
  // Native-strategierna läggs därför till i populationen, översatta till den
  // radform schemaläggaren redan förstår. De uppfinner ingenting: taggarna
  // kommer ur strategins egen signaltaxonomi.
  function asNativeRow(strategyId) {
    return {
      ...
      status: 'paper_only',
      enabled: true,
      market_group: 'futures',
      market_regime_tags: ['futures', 'mnq', 'mes'],
      timeframe: '2m',
    };
  }

  function nativeStrategyRows() {
    try {
      const nativeRegistry = require('./nativeFuturesStrategyRegistryService');
      return nativeRegistry
        .listNativeStrategies({ includeVariants: true, includeEvolved: true })
        .map((row) => asNativeRow(row.strategyId));
    } catch (_) {
      return [];
    }
  }
  ...
  /**
   * Native-uppslag för ett enskilt id.
   *
   * Behövs eftersom AI Factory Orchestrator anropar schemaläggaren med
   * `strategies: []` — den vill bara ha ett jobb för den valda kunskapsluckan,
   * inte ett för varje registrerad strategi. Med tom lista byggdes en stubbrad
   * utan marknad, och en futures-strategi fick då aktiesymboler. Luckan slås
   * därför upp direkt i native-registret.
   */
  function nativeStrategyRow(strategyId) {
    const id = safeString(strategyId);
    if (!id) return null;
    try {
      const nativeRegistry = require('./nativeFuturesStrategyRegistryService');
      return nativeRegistry.isNativeStrategyId(id) ? asNativeRow(id) : null;
    } catch (_) {
      return null;
    }
  }
  ...
  function loadStrategies(explicit) {
    ...
    let catalog = [];
    try {
      if (typeof registryService.listStrategies === 'function') catalog = registryService.listStrategies() || [];
    } catch (_) {}
    const byId = new Map(catalog.map((row) => [strategyIdOf(row), row]).filter(([id]) => id));
    for (const row of nativeStrategyRows()) {
      // Native vinner vid krock: den raden vet att strategin körs på CME.
      byId.set(row.strategy_id, { ...(byId.get(row.strategy_id) || {}), ...row });
    }
    return [...byId.values()];
  }
