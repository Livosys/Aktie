...
// Native Futures: VWAP Momentum Long
//
// Migrering av Strategy Store-strategin `vwap_momentum_long` till native
// futures-vägen. Ingen handelslogik är nyskriven. Modulen kör samma legacy-
// beslutskedja och läser ut svaret:
//
...
//
// Katalogdefinition (daytradingStrategyCatalogService, id vwap_momentum_long):
//   direction long, runtime_signal VWAP_RECLAIM_UP (family VWAP_RECLAIM_REJECTION),
//   default_stop_loss_pct 0.18, default_take_profit_r 1.6.
//   signal_rules [price_breaks_above_vwap, momentum_spike, strong_move_up] motsvaras i
//   motorn av `evaluateVwapReclaimRejection`: priset inom 0,45 % från VWAP,
//   momentumIsUsable (momentumgrinden), samt 2m-/candle-bekräftelse i riktningen.
...
const STRATEGY_ID = 'native_futures_vwap_momentum_long_v1';
const STRATEGY_VERSION = 'migration3';
const SOURCE = 'native_futures_vwap_momentum_strategy';
const ORIGIN_STRATEGY_ID = 'vwap_momentum_long';
...
function evaluateNativeFuturesVwapMomentumStrategy(snapshot, options = {}) {
  ...
  if (!matched) {
    return baseDecision(snapshot, now, DECISIONS.NO_SIGNAL, 'vwap_momentum_not_triggered', {
      evidence,
    });
  }
  ...
  return baseDecision(snapshot, now, DECISIONS.SIGNAL, 'vwap_momentum_long', {
...
function createNativeFuturesVwapMomentumStrategy(options = {}) {
  return {
    strategyId: STRATEGY_ID,
    strategyVersion: STRATEGY_VERSION,
    evaluate(snapshot, overrides = {}) {
      return evaluateNativeFuturesVwapMomentumStrategy(snapshot, { ...options, ...overrides });
    },
  };
}

const defaultNativeFuturesVwapMomentumStrategy = createNativeFuturesVwapMomentumStrategy();
...
module.exports = {
  ...
  defaultNativeFuturesVwapMomentumStrategy,
