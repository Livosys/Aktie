import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import SystemStatusStrip from './SystemStatusStrip.jsx';
import ThemeToggle from './ThemeToggle.jsx';
import AiSummaryBar from './AiSummaryBar.jsx';
import { useAuth } from '../auth/AuthProvider.jsx';

const SEARCH_ROUTES = [
  { key: 'ai fabriken', to: '/factory' },
  { key: 'fabriken', to: '/factory' },
  { key: 'nästa steg', to: '/factory' },
  { key: 'översikt', to: '/factory' },
  { key: 'oversikt', to: '/factory' },
  { key: 'strategibibliotek', to: '/factory/library' },
  { key: 'strategier', to: '/factory/library' },
  { key: 'tester', to: '/factory/replay' },
  { key: 'historiska tester', to: '/factory/replay' },
  { key: 'handelstest', to: '/futures-paper' },
  { key: 'futures', to: '/futures-paper' },
  { key: 'paper', to: '/futures-paper' },
  { key: 'mnq', to: '/futures-paper' },
  { key: 'mes', to: '/futures-paper' },
  { key: 'scanner', to: '/lab' },
  { key: 'signaler', to: '/lab' },
  { key: 'aktier', to: '/lab' },
  { key: 'nasdaq', to: '/lab' },
  { key: 'krypto', to: '/lab' },
  { key: 'larm', to: '/system?tab=logs' },
  { key: 'säkerhet', to: '/system?tab=safety' },
  { key: 'broker', to: '/system?tab=broker' },
  { key: 'datakällor', to: '/system?tab=providers' },
  { key: 'diagram', to: '/lab?tab=review' },
  { key: 'historik', to: '/lab?tab=replay' },
  { key: 'intelligens', to: '/lab?tab=adaptive' },
  { key: 'hälsa', to: '/system?tab=health' },
  { key: 'halsa', to: '/system?tab=health' },
  { key: 'system', to: '/system' },
  { key: 'lab', to: '/lab' },
  { key: 'insikter', to: '/lab?tab=replay' },
];

export default function TopBar({ onMenu, status }) {
  const [query, setQuery] = useState('');
  const navigate = useNavigate();
  const auth = useAuth();

  function submitSearch(event) {
    event.preventDefault();
    const q = query.trim().toLowerCase();
    if (!q) return;
    const route = SEARCH_ROUTES.find((item) => item.key.includes(q) || q.includes(item.key));
    if (route) {
      navigate(route.to);
      setQuery('');
      return;
    }
    navigate(`/lab?tab=review&symbol=${encodeURIComponent(query.trim().toUpperCase())}&timeframe=2m`);
    setQuery('');
  }

  function openHelp() {
    window.dispatchEvent(new CustomEvent('trading-os-help:open'));
  }

  return (
    <header className="premium-topbar">
      <div className="premium-topbar-main">
        <button className="premium-menu-button" type="button" onClick={onMenu} aria-label="Öppna meny">
          <span />
          <span />
          <span />
        </button>

        <form className="premium-search" onSubmit={submitSearch}>
          <span>Sök</span>
          <input
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder="Symbol eller sida"
            aria-label="Sök symbol eller sida"
          />
        </form>

        <SystemStatusStrip status={status} />
        <button type="button" className="premium-help-button" onClick={openHelp}>
          Ny här?
        </button>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          <span style={{ color: 'var(--muted)', fontSize: 12, fontWeight: 800 }}>
            {auth.user?.role === 'admin' ? 'Admin' : auth.user?.username || 'Trading OS'}
          </span>
          <button type="button" className="premium-help-button" onClick={auth.logout}>
            Logga ut
          </button>
        </div>
        <ThemeToggle />
      </div>
      <AiSummaryBar />
    </header>
  );
}
