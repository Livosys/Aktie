...
export function normalizeStrategyViewModel(source = {}) {
  ...
  return {
    ...
    marketRegime: firstValue(source.marketRegime, source.market_regime, source.regime, candidate?.marketRegime, overview?.marketRegime, statusSource?.marketRegime),
    ...
    blockedReason: firstValue(source.blockedReason, source.blocked_reason, source.paperBlockedReason, source.runtimeBlockedReason, source.riskSnapshot?.blockedReason, candidate?.blockedReason, candidate?.blockReason, overview?.mainBlocker),
    ...
    metadata: {
      ...
      status: firstValue(source.metadata?.status, rawStatus, source.catalogStatus, overview?.status, statusSource?.status),
